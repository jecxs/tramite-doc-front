...
export const getProcedures = async (
  ...
  try {
    const response = await apiClient.get<PaginatedResponse<Procedure>>(PROCEDURES_ENDPOINT, {
      params: filters,
    });
...
export const updateProcedureState = async (
  ...
  try {
    const response = await apiClient.patch<Procedure>(`${PROCEDURES_ENDPOINT}/${id}/estado`, data);
    return response.data;
  } catch (error) {
    throw new Error(handleApiError(error));
  }
 ...
 */
export const getProcedureHistory = async (id: string): Promise<Procedure[]> => {
  try {
    const response = await apiClient.get<Procedure[]>(`${PROCEDURES_ENDPOINT}/${id}/historial`);
    return response.data;
  } catch (error) {
    throw new Error(handleApiError(error));
  }
