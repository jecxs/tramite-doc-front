...
import { DeteccionResultado, CreateTramiteAutoLoteDto } from '@/types';
...
export const crearTramitesAutoLote = async (
  ...
  try {
    const response = await apiClient.post(`${TRAMITES_ENDPOINT}/auto-lote`, data);
...
export const generarMensajePredeterminado = async (
  ...
  try {
    const response = await apiClient.get(`${TRAMITES_ENDPOINT}/auto-lote/template/${codigoTipo}`, {
