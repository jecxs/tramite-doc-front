 ...
 */
export const getObservationsByProcedure = async (idTramite: string): Promise<Observation[]> => {
  try {
...
export const getPendingObservations = async (): Promise<Observation[]> => {
  try {
    const response = await apiClient.get<Observation[]>(`${OBSERVACIONES_ENDPOINT}/pendientes`);
    return response.data;
  } catch (error) {
    throw new Error(handleApiError(error));
  }
...
export const getObservationById = async (id: string): Promise<Observation> => {
  try {
    const response = await apiClient.get<Observation>(`${OBSERVACIONES_ENDPOINT}/${id}`);
    return response.data;
  } catch (error) {
    throw new Error(handleApiError(error));
  }
