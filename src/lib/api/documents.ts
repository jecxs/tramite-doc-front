...
export const uploadDocument = async (formData: FormData): Promise<Document> => {
  try {
    const response = await apiClient.post<Document>(`${DOCUMENTS_ENDPOINT}/upload`, formData, {
      headers: {
 ...
 */
export const getDocuments = async (params?: { search?: string; id_tipo?: string; page?: number; limit?: number }): Promise<Document[]> => {
  try {
    const response = await apiClient.get<Document[]>(DOCUMENTS_ENDPOINT, { params });
    return response.data;
  } catch (error) {
    throw new Error(handleApiError(error));
  }
 ...
 */
export const createDocumentVersion = async (id: string, formData: FormData): Promise<Document> => {
  try {
    const response = await apiClient.post<Document>(`${DOCUMENTS_ENDPOINT}/${id}/version`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
export const getDocumentDownloadUrl = async (id: string): Promise<{ url: string; expires_in: number }> => {
  try {
    const response = await apiClient.get<{ url: string; expires_in: number }>(`${DOCUMENTS_ENDPOINT}/${id}/download`);
    return response.data;
  } catch (error) {
    throw new Error(handleApiError(error));
  }
...
export const deleteDocument = async (id: string): Promise<{ message: string }> => {
  try {
    const response = await apiClient.delete<{ message: string }>(`${DOCUMENTS_ENDPOINT}/${id}`);
    return response.data;
  } catch (error) {
    throw new Error(handleApiError(error));
  }
...
export const getDocumentContentUrl = (documentId: string): string => {
  ...
  return `${config.API_URL}${DOCUMENTS_ENDPOINT}/${documentId}/content`;
};
...
export const getDocumentBlob = async (documentId: string): Promise<Blob> => {
  try {
    const response = await apiClient.get(`${DOCUMENTS_ENDPOINT}/${documentId}/content`, {
      responseType: 'blob',
    });
    return response.data;
  } catch (error) {
