...
): Promise<{ respuesta: RespuestaTramite; tramiteActualizado: Procedure }> => {
  try {
    const response = await apiClient.post<{
      respuesta: RespuestaTramite;
      tramiteActualizado: Procedure;
    }>(`${RESPUESTA_TRAMITE_ENDPOINT}/${idTramite}`, data);
 ...
 */
export const obtenerRespuestaTramite = async (idTramite: string): Promise<RespuestaTramite> => {
  try {
