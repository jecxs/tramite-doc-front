...
export default function WorkerProcedureDetailPage() {
  ...
  const fetchDocumentUrl = async (documentId: string) => {
    try {
      const proxyUrl = `${config.API_URL}/documentos/${documentId}/content`;
          ...
          <div className='flex gap-2'>
            <div className='flex gap-1 bg-gray-100 rounded-lg p-1'>
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                  viewMode === 'details' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                }`}
              >
                Detalles
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                  viewMode === 'viewer' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                }`}
              >
            ...
            {procedure.estado === 'ABIERTO' && viewMode === 'details' && (
              <Button onClick={handleMarkAsRead} disabled={isMarking} variant='outline'>
                {isMarking ? <Loader2 className='w-4 h-4 animate-spin' /> : <FileCheck className='w-4 h-4' />}
                Marcar como Leído
              </Button>
            )}
          </div>
            ...
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  ...
                  <p className='text-lg font-semibold text-gray-900'>{PROCEDURE_STATE_LABELS[procedure.estado as keyof typeof PROCEDURE_STATE_LABELS]}</p>
                </div>
              </div>
              ...
              <div className='mt-4 bg-purple-50 border border-purple-200 rounded-lg p-4'>
                <div className='flex items-start gap-3'>
                  ...
                  <div className='flex-1'>
                    <p className='text-sm font-medium text-purple-900'>Este documento requiere tu firma electrónica</p>
                    <p className='text-sm text-purple-700 mt-1'>Una vez que lo hayas leído completamente, haz clic en el botón para firmar.</p>
                    <Button onClick={() => setShowFirmaModal(true)} size='sm' className='mt-3 bg-purple-600 hover:bg-purple-700 text-white'>
            ...
            <div className='lg:col-span-2 space-y-6'>
                  ...
                  <div className='space-y-4'>
                        ...
                        {procedure.es_reenvio ? (
                          ...
                          <div className='mt-2 p-4 bg-orange-30 border-l-4 border-blue-200 rounded-r-lg'>
                            <div className='flex items-start gap-3'>
                              ...
                              <div className='flex-1'>
                                <p className='text-sm font-semibold text-blue-900 mb-2'>Documento Corregido - Versión {procedure.numero_version}</p>
                                <p className='text-sm text-gray-700 leading-relaxed'>{procedure.mensaje}</p>
                                {procedure.motivo_reenvio && (
                                  <div className='mt-3 pt-3 border-t border-orange-100'>
                                    <p className='text-xs font-medium text-green-800 mb-1'>Corrección:</p>
                                    <p className='text-xs text-gray-600 italic'>{procedure.motivo_reenvio}</p>
                                  </div>
                                )}
                              </div>
                            </div>
                          ...
                          <p className='text-sm text-gray-600 mt-1 whitespace-pre-wrap'>{procedure.mensaje}</p>
                        )}
                      </div>
                    )}
                    ...
                    <div className='grid grid-cols-2 gap-4 pt-4 border-t'>
                      <div>
                        ...
                        <p className='text-sm text-gray-900 mt-1'>{procedure.documento?.tipo?.nombre || 'N/A'}</p>
                      </div>
                      <div>
                        <label className='text-sm font-medium text-gray-700'>Código del Tipo</label>
                        <p className='text-sm text-gray-900 mt-1 font-mono'>{procedure.documento?.tipo?.codigo || 'N/A'}</p>
                      </div>
                    </div>
                    ...
                    <div className='grid grid-cols-3 gap-4 pt-4 border-t'>
                      <div>
                        ...
                        <p className='text-sm text-gray-900 mt-1'>{procedure.documento?.nombre_archivo || 'Cargando...'}</p>
                      </div>
                      <div>
                        <label className='text-sm font-medium text-gray-700'>Tamaño</label>
                        <p className='text-sm text-gray-900 mt-1'>{procedure.documento?.tamano_bytes ? formatBytes(parseInt(procedure.documento.tamano_bytes)) : 'Cargando...'}</p>
                      </div>
                      <div>
                        <label className='text-sm font-medium text-gray-700'>Extensión</label>
                        <p className='text-sm text-gray-900 mt-1 uppercase'>{procedure.documento?.extension || 'Cargando...'}</p>
                      </div>
                    </div>
              ...
              {procedure.requiere_respuesta && (
                <div className='space-y-6'>
                      ...
                      <>
                        ...
                        <div className='bg-teal-50 border border-teal-200 rounded-lg p-4'>
                          <div className='flex items-start gap-3'>
                            ...
                            <div className='flex-1'>
                              <p className='text-sm font-medium text-teal-900 mb-1'>Este documento requiere tu confirmación</p>
                              <p className='text-sm text-teal-800'>Por favor, revisa el documento completamente antes de confirmar.</p>
                            </div>
                          </div>
                        </div>
                  ...
                  {!procedure.respuesta && (procedure.estado === 'ENVIADO' || procedure.estado === 'ABIERTO') && (
                    <div className='bg-blue-50 border border-blue-200 rounded-lg p-4'>
                      <div className='flex items-center gap-3'>
                        <Info className='w-5 h-5 text-blue-600' />
                        <p className='text-sm text-blue-800'>
                          Debes leer completamente el documento antes de poder confirmar tu conformidad. El sistema detectará automáticamente cuando hayas terminado de leerlo.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <CardContent>
                    <div className='space-y-4'>
                      {procedure.observaciones.map((obs) => (
                        <div key={obs.id_observacion} className='bg-gray-50 rounded-lg p-4 border border-gray-200'>
                          <div className='flex items-start justify-between mb-2'>
                            <span className='inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-blue-100 text-blue-800'>{obs.tipo}</span>
                            <span className='text-xs text-gray-500'>{format(new Date(obs.fecha_creacion), 'dd/MM/yyyy HH:mm', { locale: es })}</span>
                          </div>
                          <p className='text-sm text-gray-900 whitespace-pre-wrap'>{obs.descripcion}</p>
                          {obs.resuelta && obs.respuesta && (
                            <div className='mt-3 pt-3 border-t border-gray-300'>
                              ...
                              {obs.fecha_resolucion && (
                                <p className='text-xs text-gray-500 mt-1'>Resuelta el {format(new Date(obs.fecha_resolucion), 'dd/MM/yyyy HH:mm', { locale: es })}</p>
                              )}
                            </div>
                          )}
                        </div>
                  ...
                  <div className='space-y-3'>
                    <div className='flex items-start gap-3'>
                      ...
                      <div className='flex-1'>
                        ...
                        <p className='text-sm text-gray-900'>{format(new Date(procedure.fecha_envio), 'dd/MM/yyyy HH:mm', { locale: es })}</p>
                      </div>
                    </div>
                    ...
                    {procedure.fecha_abierto && (
                      <div className='flex items-start gap-3'>
                        ...
                        <div className='flex-1'>
                          ...
                          <p className='text-sm text-gray-900'>{format(new Date(procedure.fecha_abierto), 'dd/MM/yyyy HH:mm', { locale: es })}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {procedure.fecha_leido && (
                      <div className='flex items-start gap-3'>
                        ...
                        <div className='flex-1'>
                          ...
                          <p className='text-sm text-gray-900'>{format(new Date(procedure.fecha_leido), 'dd/MM/yyyy HH:mm', { locale: es })}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {procedure.fecha_firmado && (
                      <div className='flex items-start gap-3'>
                        ...
                        <div className='flex-1'>
                          ...
                          <p className='text-sm text-gray-900'>{format(new Date(procedure.fecha_firmado), 'dd/MM/yyyy HH:mm', { locale: es })}</p>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className='space-y-2'>
                    {canSign && (
                      <Button onClick={() => setShowFirmaModal(true)} className='w-full bg-purple-600 hover:bg-purple-700 text-white'>
                        <PenTool className='w-4 h-4' />
                        Firmar Documento
                      </Button>
                    )}
