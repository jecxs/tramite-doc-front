...
import { NOTIFICATION_LABELS, PROCEDURE_TYPES, ROLES, ROUTE_BUILDERS } from '@/lib/constants';
...
export default function NotificationList({
  ...
  const getNotificationIcon = (tipo: string) => {
    ...
    switch (tipo) {
      case PROCEDURE_TYPES.TRAMITE_RECIBIDO:
        return <FileText className={`${iconClass} text-blue-400`} />;
      case PROCEDURE_TYPES.TRAMITE_FIRMADO:
        return <PenTool className={`${iconClass} text-green-400`} />;
      case PROCEDURE_TYPES.TRAMITE_ANULADO:
        return <XCircle className={`${iconClass} text-red-400`} />;
      case PROCEDURE_TYPES.OBSERVACION_CREADA:
        return <MessageSquare className={`${iconClass} text-orange-400`} />;
      case PROCEDURE_TYPES.OBSERVACION_RESUELTA:
        return <CheckCheck className={`${iconClass} text-green-400`} />;
      case PROCEDURE_TYPES.DOCUMENTO_REQUIERE_FIRMA:
        return <AlertTriangle className={`${iconClass} text-yellow-400`} />;
      case PROCEDURE_TYPES.TRAMITE_REENVIADO:
  ...
  if (isLoading) {
    return (
      <div className='bg-card rounded-3xl p-16 shadow-2xl border border-border backdrop-blur-sm'>
        <div className='text-center'>
          <Loader2 className='w-14 h-14 text-blue-500 animate-spin mx-auto mb-4' />
          <p className='text-muted-foreground text-lg'>Cargando notificaciones...</p>
        </div>
      </div>
    );
  }
    ...
    <div className='bg-card rounded-3xl shadow-2xl overflow-hidden border border-border backdrop-blur-sm'>
      ...
      <div className='border-b border-border px-8 py-6'>
        <div className='flex items-center justify-between mb-6'>
          <h2 className='text-2xl font-bold text-foreground'>Todas las notificaciones</h2>
          <button
            onClick={handleRefresh}
            disabled={isRefreshing}
            className='group flex items-center gap-2.5 px-5 py-2.5 bg-muted hover:opacity-90 text-foreground rounded-xl transition-all duration-300 disabled:opacity-50'
          >
            <RefreshCw
              className={clsx(
        ...
        <div className='flex gap-3'>
          ...
          >
            {filter === 'all' && (
              <div className='absolute inset-0 bg-gradient-to-r from-primary/20 to-primary/30 rounded-xl animate-pulse' />
            )}
      ...
      <div className='p-6'>
        {filteredNotifications.length === 0 ? (
          <div className='text-center py-20'>
            <div className='w-20 h-20 bg-muted rounded-2xl flex items-center justify-center mx-auto mb-5 backdrop-blur-sm border border-border'>
              <Bell className='w-10 h-10 text-muted-foreground' />
            </div>
            <p className='text-muted-foreground text-lg font-medium'>No hay notificaciones</p>
            <p className='text-muted-foreground text-sm mt-2'>Tus notificaciones aparecerán aquí</p>
          </div>
        ) : (
            ...
            {filteredNotifications.map((notification) => (
              <button
                ...
                className={clsx(
                  ...
                  !notification.visto
                    ? ['bg-card', 'border-l-4', getNotificationAccent(notification.tipo), 'border']
                    : ['bg-muted', 'border', 'border-border', 'hover:opacity-90'],
                )}
              >
                ...
                <div className='relative z-10 flex items-start gap-4'>
                  {/* Icono */}
                  <div className='flex-shrink-0 w-12 h-12 bg-input backdrop-blur-sm rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 border border-border'>
                  ...
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-start justify-between gap-3 mb-2'>
                      <h3 className='text-base font-semibold text-foreground group-hover:opacity-80 transition-colors duration-300'>
                        {notification.titulo}
                      </h3>
                      {!notification.visto && (
                    ...
                    <div className='flex items-center gap-3 text-xs'>
                      ...
                      <span className='text-muted-foreground bg-muted px-3 py-1 rounded-lg border border-border'>
                        {NOTIFICATION_LABELS[
