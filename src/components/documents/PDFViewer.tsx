...
import { Loader2, ChevronLeft, ChevronRight, Download, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function PDFViewer({
    ...
    useEffect(() => {
        if (numPages > 0) {
            ...
            if (percentage >= readThreshold && !hasMarkedAsRead && autoMarkAsRead && onReadThresholdReached) {
                handleMarkAsRead();
            }
        }
        ...
        <div className="flex flex-col h-[800px] border border-gray-200 rounded-lg overflow-hidden bg-white">
            ...
            {autoMarkAsRead && !hasMarkedAsRead && (
                <div className="px-4 py-2 bg-blue-50 border-t border-blue-200 text-xs text-blue-800">
                    💡 El documento se marcará como leído automáticamente cuando visites al menos el {readThreshold}% de las páginas
                </div>
            )}
