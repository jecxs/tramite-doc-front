...
import { ProcedureFilters, DocumentType } from '@/types';
...
export default function TramitesAdvancedFilters({
    ...
    const [selectedTipoDocumento, setSelectedTipoDocumento] = useState<string>(currentFilters.id_tipo_documento || '');
    const [selectedReceptor, setSelectedReceptor] = useState<string>(currentFilters.id_receptor || '');
    const [fechaEnvioDesde, setFechaEnvioDesde] = useState<string>(currentFilters.fecha_envio_desde || '');
    const [fechaEnvioHasta, setFechaEnvioHasta] = useState<string>(currentFilters.fecha_envio_hasta || '');
    const [tieneObservaciones, setTieneObservaciones] = useState<boolean | undefined>(currentFilters.tiene_observaciones);
    const [observacionesPendientes, setObservacionesPendientes] = useState<boolean | undefined>(currentFilters.observaciones_pendientes);
    const [conRespuesta, setConRespuesta] = useState<boolean | undefined>(currentFilters.con_respuesta);
    ...
    const loadFilterOptions = async () => {
        ...
        try {
            const [tipos, workers] = await Promise.all([
                getDocumentTypes(),
                getWorkers(),
            ]);
    ...
    const handleApply = () => {
        ...
        if (observacionesPendientes !== undefined) advancedFilters.observaciones_pendientes = observacionesPendientes;
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => setConRespuesta(e.target.value === '' ? undefined : e.target.value === 'true')}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white"
                        >
                            <option value="">Todas</option>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => setEsReenvio(e.target.value === '' ? undefined : e.target.value === 'true')}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white"
                        >
